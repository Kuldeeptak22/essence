...
export default function CategoryPage() {
  ...
  const editCategory = (_id) => {
    navigate(`/dashboard/category/editCategory/${_id}`);
  };
  const deleteCategory = (category_id) => {
    axios.delete(`${baseURL}/categories/delete_category/${category_id}`).then((res) => {
      handleClose();
      fetchData();
    });
  };
    ...
    <>
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <Button variant="contained" startIcon={<Iconify icon="eva:plus-fill" />} onClick={addCategory}>
            New Category
          </Button>
        </Stack>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { _id, name, description, image } = row;
                      ...
                      <TableRow hover key={_id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        ...
                        <TableCell component="th" scope="row" padding="none">
                          <Stack direction="row" alignItems="center" spacing={2}>
                            <Avatar alt={name} src={`${baseURL}/uploads/categories/${image}`} />
